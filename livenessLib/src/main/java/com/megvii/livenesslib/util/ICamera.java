...
public class ICamera {
    ...
    private byte[] mImageCallbackBuffer = new byte[cameraWidth
            * cameraHeight * 3 / 2];
    ...
    public Camera openCamera(Activity activity) {
        try {
            ...
            if(!ConUtil.isSupportSP("video/avc")) {
                params.setPreviewFormat(ImageFormat.YV12);
            }
            params.setPreviewSize(cameraWidth, cameraHeight);
            mCamera.addCallbackBuffer(mImageCallbackBuffer);
            int stride = (int) Math.ceil(cameraWidth / 16.0f) * 16;
            int cStride = (int) Math.ceil(cameraWidth / 32.0f) * 16;
            final int frameSize = stride * cameraWidth;
            final int qFrameSize = cStride * cameraHeight / 2;
