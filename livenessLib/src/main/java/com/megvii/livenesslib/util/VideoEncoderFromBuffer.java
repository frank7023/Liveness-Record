...
public class VideoEncoderFromBuffer {
    ...
    public boolean isSPYUV;

    @SuppressLint("NewApi")
    public VideoEncoderFromBuffer(String fileName, int width, int height) {
        Log.i(TAG, "VideoEncoder()");
        this.mWidth = width;
        this.mHeight = height;
        mFrameData = new byte[this.mWidth * this.mHeight * 3 / 2];
        int BIT_RATE = height * width * 3 * 8 * FRAME_RATE / COMPRESS_RATIO; // bit rate CameraWrapper.
        ...
        isSPYUV = isSemiPlanarYUV(mColorFormat);
        if (VERBOSE)
    ...
    public void encodeFrame(byte[] input/* , byte[] output */) {
        ...
        if (isSPYUV) {
            ...
            byte[] bytes = swapYV12toI420(input, mWidth, mHeight);
            mFrameData = bytes;
        }
